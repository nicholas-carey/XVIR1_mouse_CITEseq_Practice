...
dittoDimPlot(merged_data, var='ident', do.label = TRUE)
dev.off()
...
```{r load_new}
load(paste0(dir_prefix, "lymphoid/merged_E1_liver_lymphoid_singlets_reannotated.RData"))
load(paste0(dir_prefix, "myeloid/merged_E1_liver_myeloid_singlets_reannotated.RData"))
```
...
write.table(count_by_age_lympho, file=paste0(dir_prefix, "lymphoid/cell_type_count_by_age.csv"), sep=',')
...
write.table(count_by_age_myelo, file=paste0(dir_prefix, "myeloid/cell_type_count_by_age.csv"), sep=',')
...
```{r ages_plot}
png(filename=paste0(dir_prefix, 'lymphoid/lympho_frequency_barplot_by_age.png'), width = 5, height = 5, units = "in", res = 300)
dittoBarPlot(merged_lympho, var='anno_lympho_res1', group.by='age')+ylab("Percent of Lymphoid Cells")
dev.off()

png(filename=paste0(dir_prefix, 'myeloid/myelo_frequency_barplot_by_age.png'), width = 5, height = 5, units = "in", res = 300)
dittoBarPlot(merged_myelo, var='anno_myelo_res1', group.by='age')+ylab("Percent of Myeloid Cells")
dev.off()
...
final_anno_tidy$final_anno <- factor(final_anno_tidy$final_anno,
                                     levels=c("B cell", "Plasma cell", "IgA Plasma cell",
                                              "Naive B cell", "CD4 T cell", "Treg", "CD8 T cell", "gdT cell",
                                              "Proliferating CD8 T cell", "NK cell", "NKT cell", "ILC1",
                                              "ILC3", "cDC1", "DC", "DC3", "pDC", "Inflammatory Kuppfer Cell",
                                              "Non-inflammatory Kuppfer Cell", "Inflammatory Macrophage", 
                                              "Macrophage 4", "Monocyte", "Neutrophil", "pre-BMP", "Endothelial"))
...
ggplot(data=final_anno_tidy, aes(fill=final_anno, y=Freq, x=age))+
  ...
  scale_fill_manual(values=head(dittoColors(), nlevels(final_anno_tidy$final_anno)))+
...
for(type in unique(final_anno_by_sample$final_anno)){
  png(filename=paste0(dir_prefix, type, '_boxplot.png'), width = 5, height = 5, units = "in", res = 300)
